...
def cross(A, B):
    """
    a cross product of elements in A an in B
    """
...
def naked_twins(values):
    ...
    for box in boxes_with_two_values:
        ...
        for peer in peers[box]:
            if values[peer] == values[box]: # twin found
...
def eliminate(values):
    """
    iterate through all boxes, for every solved box, eliminate its value from the values of all
    its peers.
    :param values: values(dict): a dictionary of the form {'box_name': '123456789', ...}
    :return:
    """
...
def only_choice(values):
    """
    for every unit in unitlist, assign a value for a box if there is a unit with a value that only fits in that
    particular box.
    :param values: values(dict): a dictionary of the form {'box_name': '123456789', ...}
    :return:
    """
...
def reduce_puzzle(values):
    """
    apply all the implemented strategies (eliminate, only choice, and naked_twins) to the given puzzle
    if at some point, there is a box with an empty values, return False
    otherwise return the sudoku
    :param values: values(dict): a dictionary of the form {'box_name': '123456789', ...}
    :return:
    """
    stalled = False
...
if __name__ == '__main__':
    ...
    # display(solve(top95_1_grid, False))
