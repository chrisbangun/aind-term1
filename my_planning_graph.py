...
class PlanningGraph():
    ...
    def create_graph(self):
        ...
        while not leveled:
            ...
            if self.s_levels[level] == self.s_levels[level - 1]:  # two consecutive level are identical
                leveled = True
    ...
    def add_action_level(self, level):
        ...
        pos_literal, neg_literal = [], []
        self.a_levels.append(set())

        for S in self.s_levels[level]:
            if '~' in str(S.literal):
                neg_literal.append(S.symbol)
            else:
                pos_literal.append(S.symbol)

        for action in self.all_actions:
            is_possible = True
            for clause in action.precond_pos:
                if clause not in pos_literal:
                    is_possible = False

            for clause in action.precond_neg:
                if clause not in neg_literal:
                    is_possible = False

            if is_possible:
                self.a_levels[level].add(PgNode_a(action))
                for S in self.s_levels[level]:  # not sure
                    S.children.add(PgNode_a(action))
    ...
    def add_literal_level(self, level):
        ...
        for a_level in self.a_levels[level - 1]:
            for clause in a_level.effnodes:
                if '~' in str(clause.literal):
                    self.s_levels[level].add(PgNode_s(clause.symbol, False))
                else:
                    self.s_levels[level].add(PgNode_s(clause.symbol, True))
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for clause in node_a1.action.effect_add:
            if clause in node_a2.action.effect_rem:
                return True

        for clause in node_a1.action.effect_rem:
            if clause in node_a2.action.effect_add:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for parent_a in node_a1.parents:
            for parent_b in node_a2.parents:
                if self.inconsistent_support_mutex(parent_a, parent_b) or self.negation_mutex(parent_a, parent_b) or \
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        '''
        # print(node_s1.symbol)
        parent_actions_A = node_s1.parents
        parent_actions_B = node_s2.parents
        actions_literal_a = []
        actions_literal_b = []

        for actions_a in parent_actions_A:
            ...
            for actions_b in parent_actions_B:
    ...
    def h_levelsum(self) -> int:
        ...
        for level, s in enumerate(self.s_levels):
            for clause in s:
                if clause.literal in self.problem.goal and clause.symbol not in have_checked:
                    level_sum += level
                    have_checked.add(clause.symbol)
