    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            for S in self.s_levels[level]:
                is_applicable = True
                if S.is_pos:
                    if S.symbol not in action.precond_pos:
                        is_applicable = False
                else:
                    if S.symbol not in action.precond_neg:
                        is_applicable = False

                if is_applicable:
                    applicable_action = PgNode_a(action)
                    applicable_action.parents.add(S)
                    self.a_levels[level].add(applicable_action)
                    S.children.add(applicable_action)
    ...
    def add_literal_level(self, level):
        ...
        for a_level in self.a_levels[level - 1]:
            for clause in a_level.effnodes:
                if clause.is_pos:
                    new_literal = PgNode_s(clause.symbol, False)
                else:
                    new_literal = PgNode_s(clause.symbol, True)

                new_literal.parents.add(a_level)
                self.s_levels[level].add(new_literal)
                a_level.children.add(new_literal)
